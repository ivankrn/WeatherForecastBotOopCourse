...
/**
 * Фейковая реализация {@link ScheduledExecutorService ScheduledExecutorService} для тестирования, позволяющая вручную
 * управлять временем
 */
class FakeScheduledExecutorService extends AbstractExecutorService implements ScheduledExecutorService {
    ...
    @Override
    public <V> ScheduledFuture<V> schedule(@NonNull Callable<V> callable, long delay, @NonNull TimeUnit timeUnit) {
        return scheduleInternal(callable, delay, 0, timeUnit);
    }

    @Override
    public ScheduledFuture<?> scheduleAtFixedRate(@NonNull Runnable runnable, long delay, long period,
                                                  @NonNull TimeUnit timeUnit) {
        return scheduleInternal(Executors.callable(runnable, null), delay, Math.abs(period), timeUnit);
    }

    @Override
    public ScheduledFuture<?> scheduleWithFixedDelay(@NonNull Runnable runnable, long delay, long period,
                                                     @NonNull TimeUnit timeUnit) {
        return scheduleInternal(Executors.callable(runnable, null), delay, -Math.abs(period), timeUnit);
    }
     ...
     */
    private <V> ScheduledFuture<V> scheduleInternal(Callable<V> callable, long delay, long period,
                                                    @NonNull TimeUnit timeUnit) {
